...
A Docker image for Bayesian data analysis in R. See `cairn/DESIGN.md` for what it is
and `cairn/PROFILE.md` for the toolchain commands.

## Dev commands

- Lint: `hadolint Dockerfile`
- Build: `docker build -t rocker-bayes:dev .` (add `--build-arg BASE_TAG=resolute` for the other variant)
- Run locally: `docker compose up --build -d`, then <http://localhost:8787>

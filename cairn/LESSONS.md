...
- 2026-09-13 (M005): a GitHub Actions `run` step with no `shell:` key runs as `bash -e {0}`, so `set -uo pipefail` in the step keeps `-e` on. A failing pipeline in an assignment ends the step there, before any later check can print its own error.
